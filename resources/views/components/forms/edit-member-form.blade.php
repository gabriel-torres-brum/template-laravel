<div class="flex flex-col gap-4 p-2">
  ...
  <div class="grid grid-cols-12 gap-4">
    ...
    <div class="col-span-12 sm:col-span-6">
      <x-datetime-picker label="Data de nascimento" wire:model.lazy="item.birthday" parse-format="YYYY-MM-DD"
        display-format="DD/MM/YYYY" />
    </div>
    <div class="col-span-12 sm:col-span-6">
      <x-native-select label="Gênero" :options="['Masculino', 'Feminino']" wire:model.lazy="item.gender" />
    </div>
    <div class="col-span-12">
    ...
    <div class="col-span-12 sm:col-span-6">
      <x-native-select label="Igreja" :options="$churchs" option-label="name" option-value="id"
        wire:model.lazy="item.church_id" />
    </div>
  </div>
  <x-divider>Contato</x-divider>
  <div class="grid grid-cols-12 gap-4">
    <x-button outline label="Abrir telefones" icon="phone" class="col-span-6" onclick="$openModal('showPhones')" />
    <x-modal.card title="Telefones" align="center" wire:model.lazy="showPhones" max-width="lg">
      <div class="grid grid-cols-12 gap-4 p-4">
        @foreach ($phones as $key => $phone)
        <div class="flex items-center justify-between col-span-12 gap-2 md:col-span-6" wire:key='phone_{{$key}}'>
          <x-inputs.phone icon="phone" mask="['(##) ####-####', '(##) #####-####']"
            wire:model.lazy="phones.{{ $key }}.phone_number" class="flex-1 w-full pl-10" />
          <x-button.circle icon="trash" class="flex-none" sm negative wire:click="removePhone({{ $key }})" />
        </div>
        @endforeach
        <div class="col-span-12">
          <x-button positive icon="plus" label="Adicionar telefone" solid class="w-full" wire:click="addPhone()" />
        </div>
      </div>
      <x-slot name="footer">
    ...
    <x-button outline label="Abrir endereços" icon="book-open" class="col-span-6"
      onclick="$openModal('showAddresses')" />
    <x-modal.card title="Endereços" align="center" wire:model.lazy="showAddresses">
      <div class="grid grid-cols-12 gap-4 p-4">
        
        @foreach ($item->addresses as $key => $address)
        <div x-data="{ 'addressCollapse{{ $key }}': false }"
          class="flex flex-col col-span-12 border rounded-md border-slate-200 dark:border-slate-700" wire:key='address_{{$key}}'>
          <div class="flex items-center justify-between gap-2">
            ...
            <x-button.circle icon="trash" class="flex-none mr-2" xs negative wire:click="removeAddress({{ $key }})" />
          </div>
        ...
        <div x-data="{ 'newAddressCollapse{{ $key }}': false }"
          class="flex flex-col col-span-12 border rounded-md border-slate-200 dark:border-slate-700" wire:key='address_{{$key}}'>
          <div class="flex items-center justify-between gap-2">
            <button outline x-on:click="newAddressCollapse{{ $key }} = !newAddressCollapse{{ $key }}"
              class="inline-flex items-center justify-center w-full gap-1 p-2 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
              Endereço {{ $item->addresses->count() + ($key + 1) }}
              <x-icon x-cloak x-show="!newAddressCollapse{{ $key }}" solid name="chevron-left"
                class="flex-shrink-0 w-4 h-4" />
              <x-icon x-cloak x-show="newAddressCollapse{{ $key }}" solid name="chevron-down"
                class="flex-shrink-0 w-4 h-4" />
            </button>
            <x-button.circle icon="trash" class="flex-none mr-2" xs negative wire:click="removeAddress({{ $key }})" />
          </div>
          <div x-show="newAddressCollapse{{ $key }}" x-collapse
            class="grid grid-cols-12 gap-4 p-4 shadow border-slate-200 dark:border-slate-700">
            <div class="col-span-12">
              <x-input label="Endereço" wire:model.lazy="item.addresses.{{ $key }}.address_name" />
            </div>
        ...
        @endforeach
        <div class="col-span-12">
          <x-button positive icon="plus" label="Adicionar endereço" solid class="w-full" wire:click="addAddress()" />
        </div>
      </div>
      <x-slot name="footer">
        <div class="flex justify-end">
