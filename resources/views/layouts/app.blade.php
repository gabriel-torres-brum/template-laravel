...
<body x-data="{ sidebar: $persist(true) }">
    <x-notifications />
    <x-dialog id="delete" z-index="z-30" align="center" />
    {{-- <livewire:toasts /> --}}
    {{-- @include('layouts.preloader') --}}
    @auth
    <x-navbar />
    <x-sidebar />
    <main class="px-8 pt-20 pb-10 transition-all lg:ml-48" :class="{ 'lg:ml-48': sidebar }">
        <h3 class='mb-4 text-xl font-bold tracking-wide text-left'>
            @stack('pagetitle')
        </h3>
        {{ $slot }}
    </main>
